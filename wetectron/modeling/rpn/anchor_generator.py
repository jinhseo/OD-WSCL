...
class AnchorGenerator(nn.Module):
    ...
    def grid_anchors(self, grid_sizes, atten_logits):
        anchors = []
        batch_size = atten_logits.shape[0]
        for size, stride, base_anchors in zip(
            ...
            x = torch.topk(atten_logits.mean(0).reshape(-1).sigmoid(), round(grid_width*grid_height/4), dim=0)[1]//grid_width
            y = torch.topk(atten_logits.mean(0).reshape(-1).sigmoid(), round(grid_width*grid_height/4), dim=0)[1]%grid_width
            coor = torch.cat((x.unsqueeze(1), y.unsqueeze(1)), dim=1)
            shifts = coor.repeat(1,2).type(torch.float)
    ...
    def forward(self, image_list, feature_maps, atten_logits):
        grid_sizes = [feature_map.shape[-2:] for feature_map in feature_maps]
        #anchors_over_all_feature_maps = self.grid_anchors(grid_sizes, atten_logits)

        anchors = []
        for i, (image_height, image_width) in enumerate(image_list.image_sizes):
            anchors_in_image = []
            anchors_over_all_feature_maps = self.grid_anchors(grid_sizes, atten_logits[i])
