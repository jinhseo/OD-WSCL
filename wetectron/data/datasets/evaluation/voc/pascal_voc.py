...
import subprocess
import uuid
from .voc_eval_new import voc_eval_new
import torch
#from model.config import cfg


class pascal_voc(object):
  def __init__(self, dataset, output_folder, year, use_diff=False):
    self.dataset = dataset
    self._image_set = dataset.image_set
    self._year = year
    name = 'voc_' + year + '_' + dataset.image_set
    if use_diff:
      name += '_diff'
    self._data_path = dataset.root
  ...
  def gt_roidb(self):
    ...
    gt_roidb = [self._load_pascal_annotatio(index)
  ...
  def _write_voc_results_file(self, all_boxes):
    for cls_ind, cls in enumerate(self.classes):
      ...
      new_filename = os.path.join(self.output_folder, 'results', 'VOC' + self._year, 'Main')  # ------------------------

      if not os.path.exists(new_filename):
        os.makedirs(new_filename)
      with open(filename, 'wt') as f:
        for im_ind, index in enumerate(self._image_index):
          img_info = self.dataset.get_img_info(im_ind)
          img_width = img_info['width']
          img_height = img_info['height']

          all_boxes[im_ind] = all_boxes[im_ind].resize((img_width,img_height))

          inds = torch.where(all_boxes[im_ind].get_field('labels')==cls_ind+1)[0]
          dets = all_boxes[im_ind].bbox[inds]
          scores = all_boxes[im_ind].get_field('scores')[inds]
          ...
          for k in range(dets.shape[0]):
            f.write('{:s} {:.3f} {:.1f} {:.1f} {:.1f} {:.1f}\n'.
                    format(index, scores[k].item(),
                           dets[k, 0].item() + 1, dets[k, 1].item() + 1,
                           dets[k, 2].item() + 1, dets[k, 3].item() + 1))
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    imagesetfile = os.path.join(
      ...
      'Main',
      self._image_set + '.txt')
    cachedir = os.path.join(self.output_folder, 'annotations_cache')
